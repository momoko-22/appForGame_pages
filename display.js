...
function displayCard_gamemaster(array, classname, start_time, diff) {
    ...
    for (i = start_i; i < contents.length; i++) {
        ...
        //ゲームマスタータブの横にバッジを付ける
        const tab = document.getElementById('gamemaster-tab')
        if (typeof tab.children[0] === 'undefined') {
            const badge = document.createElement('span')
            const badgeText = document.createTextNode('NEW')
...
async function displayCard_clips(array, classname, windowY) {
    $('#'+classname).empty()
...
async function clip(event) {
    ...
    const windowY = document.documentElement.scrollTop
    displayCard_clips(clips, "clips", windowY)
...
function restartButtonClick(event) {
    ...
    if (newsTimerID == 0) {
        event.target.innerText = 'タイムラインを一時停止する' 
        ...
        event.target.innerText = 'タイムラインを再生する' 
...
function clickGamemasterTab(event) {
